...
def construct_example_training_comp_keras3() -> tff.learning.templates.LearningProcess:
  """Constructs a `tff.learning.templates.LearningProcess` via the FL API."""
  np.random.seed(0)
  ...
  return tff.learning.algorithms.build_weighted_fed_avg(
      model_fn,
      client_optimizer_fn=lambda: keras.optimizers.SGD(learning_rate=0.01),
  )
...
class MapReduceFormTest(tf.test.TestCase):
  ...
  def test_map_reduce_form_with_learning_structure_contains_only_one_broadcast_and_one_aggregate_keras3(
      self,
  ):
    ip = construct_example_training_comp_keras3()
  ...
  def test_map_reduce_form_with_learning_structure_does_not_change_execution_of_iterative_process_keras3(
      self,
  ):
    if tf.config.list_logical_devices('GPU'):
      self.skipTest(
    ...
    ip_1 = construct_example_training_comp_keras3()
    # We disable Grappler to prevent a single TF function from being pulled into
    # the eager TF runtime with multiple definitions.
    grappler_config = tf.compat.v1.ConfigProto()
    grappler_config.graph_options.rewrite_options.disable_meta_optimizer = True
