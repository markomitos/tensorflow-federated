...
class CustomMeanMetricKeras3(keras.metrics.Mean):
  """A custom metric whose result is total/(count + initial_count)."""

  def __init__(
      self, initial_count: float, name='custom_mean_metric_keras3', dtype=None
  ):
    self._initial_count = initial_count
    super().__init__(name=name, dtype=dtype)
    self.count.assign(initial_count)
...
class CustomSumMetricKeras3(keras.metrics.Sum):
  ...
  def __init__(
      self, has_extra_variables: bool, name='custom_sum_metric_keras3', dtype=None
  ):
    super().__init__(name=name, dtype=dtype)
    self._has_extra_variables = has_extra_variables
    if self._has_extra_variables:
...
class CustomCounterKeras3(keras.metrics.Sum):
  """A custom `tkeras.metrics.Metric` with extra arguments in `__init__`."""

  def __init__(self, name='new_metric_keras3', arg1=0, dtype=tf.int64):
    super().__init__(name, dtype)
    self._arg1 = arg1
...
class FinalizerTest(parameterized.TestCase, tf.test.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      ('keras3_metric', keras.metrics.SparseCategoricalAccuracy()),
      ('custom_keras3_metric', CustomMeanMetricKeras3(initial_count=0.0)),
      ('keras3_metric_constructor', keras.metrics.SparseCategoricalAccuracy),
  )
  ...
  @parameterized.named_parameters(
    ...
    (
        'one_variable_keras3',
        CustomSumMetricKeras3(has_extra_variables=False),
        [tf.constant(1.0)],
        computation_types.StructWithPythonType([np.float32], list),
        1.0,
    ),
    (
        'three_variables_keras3',
        CustomSumMetricKeras3(has_extra_variables=True),
        [tf.constant(1.0), tf.constant(1.0), tf.constant([1.0, 1.0])],
        computation_types.StructWithPythonType(
          [
            np.float32,
  ...
  @parameterized.named_parameters(
      ...
      (
          'loss_constructor_keras3',
          keras.losses.MeanSquaredError,
          'found a callable',
      ),  # go/pyformat-break
      (
          'custom_metric_with_extra_init_args_keras3',
          CustomCounterKeras3(arg1=1),
          'extra arguments',
      ),
  )
  ...
  def test_keras3_metric_finalizer_fails_with_unmatched_unfinalized_metric_values(
      self, invalid_unfinalized_metric_type, error_type, error_message
  ):
    # The expected unfinalized metric values for `SparseCategoricalAccuracy` is
    # a list of two `tf.Tensor`s and each has shape () and dtype tf.float32.
