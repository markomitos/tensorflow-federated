...
class ModelDeltaClientWorkComputationTest(
  ...
  def test_initialize_has_expected_type_signature_with_keras3_optimizer(
      self, weighting
  ):
    optimizer_fn = lambda: keras.optimizers.SGD(learning_rate=1.0)
    model_fn = model_examples.LinearRegression
  ...
  def test_next_has_expected_type_signature_with_keras3_optimizer(
      self, weighting
  ):
    optimizer_fn = lambda: keras.optimizers.SGD(learning_rate=1.0)
    model_fn = model_examples.LinearRegression
  ...
  def test_get_hparams_has_expected_type_signature_with_keras3_optimizer(
      self, weighting
  ):
    optimizer = lambda: keras.optimizers.SGD(learning_rate=1.0)
    model_fn = model_examples.LinearRegression
  ...
  def test_set_hparams_has_expected_type_signature_with_keras3_optimizer(
      self, weighting
  ):
    optimizer = lambda: keras.optimizers.SGD(learning_rate=1.0)
    model_fn = model_examples.LinearRegression
  ...
  def test_raises_with_created_keras3_optimizer(self):
    with self.assertRaises(TypeError):
      model_delta_client_work.build_model_delta_client_work(
          model_examples.LinearRegression,
...
class ModelDeltaClientWorkExecutionTest(
  ...
  def test_client_tf_with_keras_3_optimizer(
      self, simulation, optimizer_kwargs, expected_norm, weighting
  ):
    client_tf = (
        model_delta_client_work.build_model_delta_update_with_keras_optimizer(
  ...
  def test_correct_update_weight_with_traced_function_with_keras3_optimizer(self):
    client_tf = (
        model_delta_client_work.build_model_delta_update_with_keras_optimizer(
            model_fn=create_model,
            weighting=client_weight_lib.ClientWeighting.NUM_EXAMPLES,
  ...
  @parameterized.named_parameters(
      ...
      ('keras3_simple', lambda: keras.optimizers.SGD(learning_rate=1.0)),
      (
          'keras3_momentum',
          lambda: keras.optimizers.SGD(learning_rate=1.0, momentum=0.9),
      ),
  )
  ...
  def test_get_hparams_returns_expected_result_with_keras3_optimizer(self):
    optimizer = lambda: keras.optimizers.SGD(learning_rate=1.0, momentum=0.9)
    client_work_process = model_delta_client_work.build_model_delta_client_work(
        create_model,
        optimizer,
        client_weighting=client_weight_lib.ClientWeighting.NUM_EXAMPLES,
  ...
  def test_set_hparams_returns_expected_result_with_keras3_optimizer(self):
    optimizer = lambda: keras.optimizers.SGD(learning_rate=1.0, momentum=0.9)
    client_work_process = model_delta_client_work.build_model_delta_client_work(
        create_model,
        optimizer,
...
class FunctionalModelDeltaClientWorkExecutionTest(
  ...
  def test_functional_model_matches_model_fn_keras3(self, weighting):
    dataset = create_test_dataset()
  ...
  def _create_dataset_and_functional_model_keras3(self):
    keras_model = model_examples.build_linear_regression_keras3_functional_model(
        feature_dims=2
    )
    loss_fn = keras.losses.MeanSquaredError()
    ...
    def build_metrics_fn():
      return collections.OrderedDict(num_examples=counters.Keras3NumExamplesCounter())
  ...
  def test_metrics_output_keras3(self):
    dataset, functional_model = self._create_dataset_and_functional_model_keras3()
    process = model_delta_client_work.build_functional_model_delta_client_work(
        model=functional_model,
        optimizer=sgdm.build_sgdm(learning_rate=1.0),
        client_weighting=client_weight_lib.ClientWeighting.NUM_EXAMPLES,
  ...
  @parameterized.named_parameters(('_inf', np.inf), ('_nan', np.nan))
  def test_metrics_output_with_non_finite_updates_keras3(self, bad_value):
    dataset, functional_model = self._create_dataset_and_functional_model_keras3()
    process = model_delta_client_work.build_functional_model_delta_client_work(
        model=functional_model,
        optimizer=sgdm.build_sgdm(learning_rate=1.0),
