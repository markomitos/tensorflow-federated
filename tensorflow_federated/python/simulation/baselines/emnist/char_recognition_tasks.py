...
def _get_character_recognition_keras3_model(
    model_id: Union[str, CharacterRecognitionModel],
    only_digits: bool,
    debug_seed: Optional[int] = None,
) -> keras.Model:
  """Constructs a `keras.Model` for character recognition."""
  try:
    model_enum = CharacterRecognitionModel(model_id)
  except ValueError as e:
    raise ValueError(
  ...
  if model_enum == CharacterRecognitionModel.CNN_DROPOUT:
    keras_model = emnist_models.create_conv_dropout_keras3_model(
        only_digits=only_digits, debug_seed=debug_seed
    )
  elif model_enum == CharacterRecognitionModel.CNN:
    keras_model = emnist_models.create_original_fedavg_cnn_keras3_model(
        only_digits=only_digits, debug_seed=debug_seed
    )
  elif model_enum == CharacterRecognitionModel.TWO_LAYER_DNN:
    keras_model = emnist_models.create_two_hidden_layer_keras3_model(
        only_digits=only_digits, debug_seed=debug_seed
    )
  else:
    raise ValueError(
...
def create_character_recognition_task_from_datasets_keras3(
    train_client_spec: client_spec.ClientSpec,
    eval_client_spec: Optional[client_spec.ClientSpec],
    model_id: Union[str, CharacterRecognitionModel],
    only_digits: bool,
  ...
  def model_fn() -> variable.VariableModel:
    return keras_utils.from_keras_model(
        keras_model=_get_character_recognition_keras3_model(
            model_id, only_digits, debug_seed
        ),
...
def create_character_recognition_task_keras3(
    train_client_spec: client_spec.ClientSpec,
    eval_client_spec: Optional[client_spec.ClientSpec] = None,
    model_id: Union[str, CharacterRecognitionModel] = 'cnn_dropout',
    only_digits: bool = False,
  ...
  *   `model_id = 2nn`: A densely connected network with 2 hidden layers, each
  ...
  return create_character_recognition_task_from_datasets_keras3(
      train_client_spec,
      eval_client_spec,
      model_id,
      only_digits,
