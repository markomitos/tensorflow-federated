...
class _DeterministicInitializer:
  ...
  def __init__(
      self,
      initializer_type: Union[type[tf_keras.initializers.Initializer], type[keras.initializers.Initializer]],
      base_seed: int,
  ):
...
def create_conv_dropout_keras3_model(
    only_digits: bool = True, debug_seed: Optional[int] = None
) -> keras.Model:
  """Create a convolutional network with dropout.
  ...
  data_format = 'channels_last'
  glorot_uniform = _DeterministicInitializer(
      keras.initializers.GlorotUniform, base_seed=debug_seed
  )
  model = keras.models.Sequential([
      ...
      keras.layers.MaxPool2D(pool_size=(2, 2), data_format=data_format),
...
def create_original_fedavg_cnn_keras3_model(
    only_digits: bool = True, debug_seed: Optional[int] = None
) -> keras.Model:
  """Create a convolutional network without dropout.
...
def create_two_hidden_layer_keras3_model(
    only_digits: bool = True,
    hidden_units: int = 200,
    debug_seed: Optional[int] = None,
) -> keras.Model:
...
def create_autoencoder_keras3_model(
    debug_seed: Optional[int] = None,
) -> keras.Model:
  """Create a bottleneck autoencoder model for use with EMNIST.
