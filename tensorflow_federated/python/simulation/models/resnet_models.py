...
def _norm_relu(input_tensor, norm):
  ...
  if norm is NormLayer.GROUP_NORM:
    x = tf_keras.layers.GroupNormalization(axis=channel_axis)(input_tensor)
  ...
  return tf_keras.layers.Activation('relu')(x)


def _norm_relu_keras3(input_tensor, norm):
  """Applies normalization and ReLU activation to an input tensor.
  ...
  if norm is NormLayer.GROUP_NORM:
    x = keras.layers.GroupNormalization(axis=channel_axis)(input_tensor)
  elif norm is NormLayer.BATCH_NORM:
...
def _conv_norm_relu(input_tensor, filters, kernel_size, norm, strides=(1, 1)):
  ...
  x = tf_keras.layers.Conv2D(
      ...
      kernel_regularizer=tf_keras.regularizers.l2(L2_WEIGHT_DECAY),
  )(input_tensor)
  return _norm_relu(x, norm=norm)


def _conv_norm_relu_keras3(input_tensor, filters, kernel_size, norm, strides=(1, 1)):
  """Applies convolution, normalization, and ReLU activation to an input tensor.
  ...
  return _norm_relu_keras3(x, norm=norm)
...
def _norm_relu_conv_keras3(input_tensor, filters, kernel_size, norm, strides=(1, 1)):
  """Applies normalization, ReLU activation, and convolution to an input tensor.
  ...
  Returns:
    A `tf.Tensor`.
  """
  x = _norm_relu_keras3(input_tensor, norm=norm)
  x = keras.layers.Conv2D(
      filters,
      kernel_size,
      strides=strides,
...
def _shortcut(input_tensor, residual, norm):
  ...
  Returns:
    A `tf.Tensor`.
  """
  input_shape = tf_keras.backend.int_shape(input_tensor)
  residual_shape = tf_keras.backend.int_shape(residual)
  ...
  if stride_width > 1 or stride_height > 1 or not equal_channels:
    ...
    if norm is NormLayer.GROUP_NORM:
      shortcut = tf_keras.layers.GroupNormalization(axis=channel_axis)(shortcut)
    elif norm is NormLayer.BATCH_NORM:
      shortcut = tf_keras.layers.BatchNormalization(
          axis=channel_axis,
  ...
  return tf_keras.layers.add([shortcut, residual])


def _shortcut_keras3(input_tensor, residual, norm):
  """Computes the output of a shortcut block between an input and residual.
  ...
  Returns:
    A `tf.Tensor`.
  """
  input_shape = keras_compat.int_shape(input_tensor)
  residual_shape = keras_compat.int_shape(residual)
  ...
  if stride_width > 1 or stride_height > 1 or not equal_channels:
    ...
    if norm is NormLayer.GROUP_NORM:
      shortcut = keras.layers.GroupNormalization(axis=channel_axis)(shortcut)
    elif norm is NormLayer.BATCH_NORM:
...
def _basic_block_keras3(
  ...
  return _shortcut_keras3(input_tensor, x, norm=norm)
...
def create_resnet(
  ...
  if initial_max_pooling:
    x = tf_keras.layers.MaxPooling2D(
  ...
  x = tf_keras.layers.Dense(
      ...
      kernel_regularizer=tf_keras.regularizers.l2(L2_WEIGHT_DECAY),
      bias_regularizer=tf_keras.regularizers.l2(L2_WEIGHT_DECAY),
  )(x)

  model = tf_keras.models.Model(img_input, x)
  return model


def create_resnet_keras3(
    input_shape: tuple[int, int, int],
    num_classes: int = 10,
    residual_block: ResidualBlock = ResidualBlock.BOTTLENECK,
    repetitions: Optional[list[int]] = None,
  ...
  if residual_block is ResidualBlock.BASIC:
    block_fn = _basic_block_keras3
  elif residual_block is ResidualBlock.BOTTLENECK:
    block_fn = _bottleneck_block_keras3
  else:
    raise ValueError('residual_block must be of type `ResidualBlock`.')
  ...
  # Final activation in the residual blocks
  x = _norm_relu_keras3(x, norm=norm_layer)
